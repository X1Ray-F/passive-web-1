...
function layout(html, fm, baseHref='/'){
...
function buildIndex(posts){
  ...
  write(path.join(PUBLIC, 'index.html'), layout(html, {title:'Home', description:'Passive content site'}));
}
...
function processAll(){
  ...
  for (const f of files){
    ...
    const page = layout(`<article><h1>${data.title||''}</h1>${html}<p><a class="btn" href="https://paypal.me/FlaviusEduard98" target="_blank" rel="noopener">Cumpară / Donează</a></p></article>`, data);
